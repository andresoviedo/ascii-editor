...
function getTextWidth(ctx:CanvasRenderingContext2D, font:string){
	ctx.font = font;
...
function getTextHeight(ctx: CanvasRenderingContext2D, font: string, left:number, top:number, width:number, height:number):[number, number] {
    ...
    var data = ctx.getImageData(left, top, width, height).data,
        first: number | boolean = false,
        last: number | boolean = false,
        r = height,
        c = 0;
...
function drawBorder(canvasContext: CanvasRenderingContext2D, width:number, height: number){
...
function paint(ctx: CanvasRenderingContext2D,font:string,cellWidth:number){
	ctx.font = font;
...
function delegateProxy<T, K extends keyof T>(target:T,delegateName:K): T & T[K]{
...
function delegateByProxy(target:any,delegateName:any){
	// proxy handler
	var proxyHandler:ProxyHandler<any> = {
